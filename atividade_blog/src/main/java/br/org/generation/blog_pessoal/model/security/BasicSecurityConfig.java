package br.org.generation.blog_pessoal.model.security;
...
public class BasicSecurityConfig extends WebSecurityConfigurerAdapter  {
	...
	public PasswordEncoder passwordEncoder() {
		return new BCryptPasswordEncoder(); //BCryptPassword - Algoritmo de criptografia de senha.
	}
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		/**
		 * antMatchers().permitAll -> Endpoint liberado de autenticação -  métodos liberados 
		 * para qualquer um que acessar o sistema. 
		 * 
		 ...
		 * serão autenticados - tudo em diante deverá ser autorizado com login e senha. 
		 * 
		 ...
		 * A requisição funciona sozinha ... 
		 * não precisa rodar uma requisição anterior para rodar uma outra ... 
		 * cada uma pode ser requistada quando bem entender 
		 * (não preciso requisitar o getAll antes do getId, posso requisitar getId direto). 
		 * 
		 ...
		 * aplicação web confia. Cross-site request forgery
			Precaução contra ataque hacker. Precisa ser desabilitado porque senão o get e o set não funciona. 
			Ao invés disso, criptografamos o próprio token. 
		 * 
		...
		http.authorizeRequests()
			...
			.antMatchers(HttpMethod.OPTIONS).permitAll() //Por que liberar HttpMethod? Porque se não o Hiroko não 
			.anyRequest().authenticated()				//liberaria o cabeçalho.
			.and().httpBasic()
